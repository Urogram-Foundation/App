...
```好的，我已经查看了你提供的 `UpdateUtils.kt` 文件内容，并解决了其中的合并冲突。

考虑到你的项目名称是 "Nnngram"，并且你的本地分支 (`HEAD`) 中定义了与此相关的更新通道名称，我选择保留了本地分支 (`HEAD`) 中关于 `previewMetadataChannelID`, `previewMetadataChannelName`, `previewChannelAPKsID`, `previewChannelAPKsName` 的定义。同时保留了本地分支中被注释掉的 `stable` 通道定义，作为参考。

以下是解决冲突后的文件内容。我已经移除了 `<<<<<<<`, `=======`, `>>>>>>>` 这些冲突标记。
